...
echo "Generando {$cantidad} solicitudes para aprobaciones de compra..." . PHP_EOL;
...
for ($i = 1; $i <= $cantidad; $i++) {
	...
	DB::transaction(function () use (
		...
		$solicitud = SolicitudCompra::query()->create([
			...
			'para_ser_usado_en' => 'Prueba de flujo hacia aprobaciones y sumarios',
		...
		for ($item = 1; $item <= $itemsPorSolicitud; $item++) {
			...
			SolicitudCompraItem::query()->create([
				...
				'item' => $item,
				'descripcion' => "ITEM PRUEBA {$i}-{$item} PARA APROBACION",
				'unidad_medida' => 'UND',
...
echo PHP_EOL . 'Solicitudes creadas (pendientes en etapa Procura):' . PHP_EOL;
...
echo PHP_EOL . 'Listo. En Aprobaciones de Compra (rol Procura) deberias ver las solicitudes para firmar.' . PHP_EOL;
