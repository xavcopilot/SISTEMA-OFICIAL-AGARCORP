    ...
    public function __invoke(InventoryMovement $inventoryMovement)
    {
        ...
        try {
            ...
            $wasConvertedByLibreOffice = $this->libreOfficePdfConverter->convertSpreadsheetToPdf(
                ...
                $pdfPath,
                $tmpDir,
                ['documento' => 'formato_salida']
            );
            if (! $wasConvertedByLibreOffice) {
                \Illuminate\Support\Facades\Log::warning('Fallo conversion LibreOffice para Salida. No se usara fallback PDF.', [
                    'inventory_movement_id' => $inventoryMovement->id,
                ]);

                abort(Response::HTTP_INTERNAL_SERVER_ERROR, 'No se pudo generar el PDF del formato de salida porque LibreOffice no pudo convertir el archivo.');
            }

            if (! file_exists($pdfPath) || filesize($pdfPath) < 100) {
                \Illuminate\Support\Facades\Log::error('PDF de salida generado pero vacio o demasiado pequeno.', [
                    'was_converted_by_libreoffice' => $wasConvertedByLibreOffice,
                    'pdf_size' => file_exists($pdfPath) ? filesize($pdfPath) : 0,
                ]);

                abort(Response::HTTP_INTERNAL_SERVER_ERROR, 'No se pudo generar el PDF del formato de salida. Verifique que LibreOffice este instalado en el servidor.');
            }
