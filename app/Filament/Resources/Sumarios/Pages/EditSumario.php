...
class EditSumario extends EditRecord
{
    ...
    protected function mutateFormDataBeforeSave(array $data): array
    {
        ...
        $data['proveedor_ganador_id'] = $this->resolveWinnerProviderId($data);
    ...
    protected function handleRecordUpdate(Model $record, array $data): Model
    {
        ...
        return DB::transaction(function () use ($sumario, $data, $rows, $proveedorA, $proveedorB, $proveedorC): Sumario {
            ...
            foreach ($rows as $row) {
                ...
                $this->createOption($sumarioItem, 1, $proveedorA, $row['marca_prov1'] ?? null, (float) ($row['precio_unitario_prov1'] ?? 0), (float) ($row['precio_total_prov1'] ?? 0));
                $this->createOption($sumarioItem, 2, $proveedorB, $row['marca_prov2'] ?? null, (float) ($row['precio_unitario_prov2'] ?? 0), (float) ($row['precio_total_prov2'] ?? 0));
                $this->createOption($sumarioItem, 3, $proveedorC, $row['marca_prov3'] ?? null, (float) ($row['precio_unitario_prov3'] ?? 0), (float) ($row['precio_total_prov3'] ?? 0));
            }
    ...
    private function createOption(SumarioItem $sumarioItem, int $numero, string $proveedorNombre, ?string $marca, float $precioUnitario, float $precioTotal): void
    {
