    ...
    <div class="grid gap-4">
        <div class="rounded-2xl border border-slate-200 bg-white p-4">
            ...
            <p class="mt-1 text-sm text-slate-600">
                Ventanas monitoreadas: {{ $notificationModules }} | Con pendientes visibles: {{ $activeModules }} | Modulos con pendientes: {{ $pendingModules }}
            </p>
            ...
            <div class="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
                @foreach ($moduleTotals as $module)
                    <div class="rounded-xl border border-slate-100 bg-slate-50 px-3 py-2">
                        <div class="text-xs font-semibold uppercase tracking-wide text-slate-500">{{ $module['module'] }}</div>
                        <div class="mt-1 text-lg font-semibold {{ $module['hasPending'] ? 'text-amber-600' : 'text-slate-700' }}">
                            {{ $module['count'] }}
                        </div>
                    </div>
                @endforeach
                ...
                @forelse ($modules as $module)
                    ...
                    >
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <div class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">{{ $module['module'] }}</div>
